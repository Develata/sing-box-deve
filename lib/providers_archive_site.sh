...
sbd_write_archive_gateway_site() (
  local site_dir owner now style_file manifest_file checksums_file destination relative
  set -e
  destination="$(sbd_archive_site_dir)"
  site_dir="$(mktemp -d)" || return 1
  trap 'rm -rf "$site_dir"' EXIT
  owner="$(sbd_archive_site_owner)"
  now="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
  mkdir -p "$site_dir/assets" "$site_dir/files" || return 1

  style_file="${site_dir}/assets/style.css"
  cat > "$style_file" <<'EOF' || return 1
...
EOF
  ...
  cat > "${site_dir}/index.html" <<EOF || return 1
...
EOF
  ...
  cat > "${site_dir}/archive.html" <<EOF || return 1
...
EOF
  ...
  cat > "${site_dir}/knowledge.html" <<EOF || return 1
...
EOF
  ...
  cat > "${site_dir}/about.html" <<EOF || return 1
...
EOF
  ...
  cat > "${site_dir}/status.html" <<EOF || return 1
...
EOF
  ...
  cat > "${site_dir}/404.html" <<EOF || return 1
...
EOF
  ...
  cat > "${site_dir}/robots.txt" <<EOF || return 1
User-agent: *
Allow: /
Disallow: /private
Disallow: /tmp
...
EOF
  ...
  cat > "${site_dir}/sitemap.xml" <<EOF || return 1
...
EOF
  ...
  while IFS= read -r relative; do
    ...
    sbd_host_file_publish "$destination/$relative" "$site_dir/$relative" || return 1
  done < <(cd "$site_dir" && find . -type f -printf '%P\n' | LC_ALL=C sort)
  printf 'SBD_ARCHIVE_SITE_DIR=%s\n' "$destination" > "${SBD_DATA_DIR}/archive_site.env"
  log_info "$(msg "已生成归档网关静态站: ${destination}" "Archive gateway site generated: ${destination}")"
)
