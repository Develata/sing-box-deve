...
menu_main_descriptions() {
  ...
  printf '%s\n' "$(msg " 5 出站策略管理：选择流量路由、配置上游节点" " 5 Egress: choose traffic routing and configure upstream nodes")"
  printf '%s\n' "$(msg " 6 服务管理：重启核心与 Argo、刷新节点、看日志" " 6 Service: restart core/argo, regenerate nodes, view logs")"
  printf '%s\n' "$(msg " 7 更新管理：更新脚本发行包或内核" " 7 Update: update script release or core")"
  ...
  printf '%s\n' "$(msg "12 订阅与分享：刷新订阅、展示二维码、同步内置规则集" "12 Subscriptions: refresh, QR display, sync bundled rulesets")"
