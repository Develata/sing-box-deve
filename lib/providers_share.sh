...
share_filter_supported_links() {
  local src="$1" out="$2"
  awk '
    /^(vless|trojan|ss|hysteria2|tuic|anytls):\/\// || /^naive\+https:\/\// {print}
  ' "$src" > "$out"
}
...
share_generate_groups() {
  ...
  share_build_group_file '^(vless|trojan|ss|hysteria2|tuic|anytls)://|^naive\+https://' "$SBD_SHARE_GROUP_DIR/v2rayn.txt"
  share_build_group_file '^(vless|trojan|ss|hysteria2|tuic)://' "$SBD_SHARE_GROUP_DIR/v2rayng.txt"
  share_build_group_file '^(vless|trojan|ss|hysteria2|tuic|anytls)://|^naive\+https://' "$SBD_SHARE_GROUP_DIR/nekobox.txt"
  share_build_group_file '^(vless|trojan|ss)://' "$SBD_SHARE_GROUP_DIR/shadowrocket.txt"
  share_build_group_file '^(vless|trojan|ss|hysteria2|tuic|anytls)://|^naive\+https://' "$SBD_SHARE_GROUP_DIR/singbox.txt"
