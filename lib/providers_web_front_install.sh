...
sbd_install_official_nginx_apt() {
  ...
  key_fprs="$(gpg --show-keys --with-colons --fingerprint /usr/share/keyrings/nginx-archive-keyring.gpg 2>/dev/null | awk -F: '$1 == "fpr" {print $10}')"
  if ! grep -qx '573BFD6B3D8FBC641079A6ABABF5BD827BD9BF62' <<<"$key_fprs"; then
